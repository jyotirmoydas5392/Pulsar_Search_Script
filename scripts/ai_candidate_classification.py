...
# --------------------------------------------------
def split_and_copy_candidates(final_file, pfd_dir, output_dir, threshold):
    ...
    with open(final_file, "r") as f:
        for line in f:
            ...
            # full AR path
            pfd_path = os.path.join(pfd_dir, pfd_name)
            if not os.path.exists(pfd_path):
                logging.warning("Missing AR file: %s" % pfd_path)
                continue

            # basename without .pfd
            base = os.path.splitext(pfd_name)[0]

            # match ALL files with same base name
            pattern = os.path.join(pfd_dir, base + ".*")
...
# --------------------------------------------------
def run_ai_classifier(input_dir, output_dir, threshold):
    if not os.path.isdir(output_dir):
        os.makedirs(output_dir)

    pfd_files = glob.glob(os.path.join(input_dir, "*.pfd"))
    if not pfd_files:
        logging.warning("No .pfd files found in %s" % input_dir)
        return
    ...
    for tag, model_file in PICS_MODELS.iteritems():
        ...
        for pfd_file in pfd_files:
            try:
                pfd_data = pfdreader(pfd_file)
                score = clf.report_score([pfd_data])[0]
                fout.write("%s %s\n" % (os.path.basename(pfd_file), score))
            except Exception as e:
                logging.error("%s failed on %s: %s"
                              % (tag, os.path.basename(pfd_file), str(e)))
